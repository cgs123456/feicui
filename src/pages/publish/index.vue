      ...
      <div v-show="currentStep === 3" class="step-body">
        <div class="card preview-card">
          ...
          <div class="preview-images" v-if="images.length > 0">
            <van-image
              v-for="(img, idx) in images.length > 0 ? [images[0].url || images[0].content] : []"
              :key="idx"
              :src="images[0]?.url || images[0]?.content"
              width="200"
              height="200"
              fit="cover"
              radius="8"
...
function publish() {
  const imageUrls = images.value.filter(f => f.url || f.content).map(f => f.url || f.content)
  const coverUrl =
    imageUrls[0] ||
  ...
  const certMap = { 玻璃种: 'NGTC国检证书', 帝王绿: 'CNAS国际互认证书', 冰种: '省级鉴定证书' }
  ...
  productStore.addProduct({
    ...
    cover: coverUrl,
    images: imageUrls.length > 0 ? imageUrls : [coverUrl],
